...
def generate_therapist_reply(emotion, user_text, previous_messages=None):
    ...
    if len(user_text.split()) <= 3:
        reflect = "Could you tell me a little more about what’s happening?"
    else:
        reflect_template = random.choice(REFLECTIONS)

        if "{context}" in reflect_template:
            reflect = reflect_template.format(context=user_text)
        else:
